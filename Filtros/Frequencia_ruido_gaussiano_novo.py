...
def ler():
    ...
    for folder in os.listdir(raiz):
        ...
        for file in files:
            ...
            salEpimenta10 = cv2.cvtColor(sementes, cv2.COLOR_BGR2GRAY)  
            salEpimenta1 =noisy(salEpimenta10.astype(np.uint8)) 
            ...
            # plt.title("imagem")
            # plt.show()
            # cv2.imwrite("E:/Dissertacao/RuidoGaussiano/Imagens/FrequenciaIdeal_Imagem_Original/"+str(folder)+"/"+str(file) + ".bmp", gaussino)
            ...
            # with open('E:/Dissertacao/RuidoGaussiano/Metrica/FrequenciaIdeal/'+str(folder)+'/psnr/'+str(file)+'.txt','w') as arq2:
            #     arq2.write(str(d))
            # del(arq2)
            # with open('E:/Dissertacao/RuidoGaussiano/Metrica/FrequenciaIdeal/'+str(folder)+'/mse/'+str(file)+'.txt','w') as arq2:
            #     arq2.write(str(d1))
            # del(arq2)
            # with open('E:/Dissertacao/RuidoGaussiano/Metrica/FrequenciaIdeal/'+str(folder)+'/ssim/'+str(folder)+str(file)+'.txt','w') as arq2:
            #     arq2.write(str(d2))
            # del(arq2)           
...
#            salEpimenta1 = cv2.cvtColor(salEpimenta1, cv2.COLOR_BGR2GRAY)  
            ...
            gaussino_ruido=filtroFrequenciaBandaIdealLivro(gaussino_ruidoAlto,baixarfeq_ruido)
            ...
            cv2.imwrite("E:/Dissertacao/RuidoGaussiano/Imagens/FrequenciaIdeal_Imagem_Ruido/"+str(folder)+"/"+str(file) + ".bmp", gaussino_ruido)
                               
            d1 = np.mean((imgROI - gaussino_ruido) ** 2, dtype=np.float64)
            d = 10 * np.log10((255 ** 2) / d1)
            d2 = ssim4(imgROI.astype(np.uint8), gaussino_ruido.astype(np.uint8),multichannel=True)
            
            with open('E:/Dissertacao/RuidoGaussiano/Metrica1/FrequenciaIdeal_ruido/'+str(folder)+'/psnr/'+str(file)+'.txt','w') as arq2:
                arq2.write(str(d))
            del(arq2)
            with open('E:/Dissertacao/RuidoGaussiano/Metrica1/FrequenciaIdeal_ruido/'+str(folder)+'/mse/'+str(file)+'.txt','w') as arq2:
                arq2.write(str(d1))
            del(arq2)
            with open('E:/Dissertacao/RuidoGaussiano/Metrica1/FrequenciaIdeal_ruido/'+str(folder)+'/ssim/'+str(folder)+str(file)+'.txt','w') as arq2:
                arq2.write(str(d2))
            del(arq2)  
