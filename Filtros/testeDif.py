...
def ler():
    ...
    for folder in os.listdir(raiz):
        ...
        for file in files:
            ...
            d = psnr4(imgROI.astype(np.uint8), difusao.astype(np.uint8))
            d1 = mse4(imgROI.astype(np.uint8), difusao.astype(np.uint8))
            d2 = ssim4(imgROI.astype(np.uint8), difusao.astype(np.uint8),multichannel=True)
            
            with open('E:/Dissertacao/Resultados_Metricas/Filtro/Difusao/'+str(folder)+'/psnr/'+str(file)+'.txt','w') as arq2:
                arq2.write(str(d))
            del(arq2)
            with open('E:/Dissertacao/Resultados_Metricas/Filtro/Difusao/'+str(folder)+'/mse/'+str(file)+'.txt','w') as arq2:
                arq2.write(str(d1))
            del(arq2)
            with open('E:/Dissertacao/Resultados_Metricas/Filtro/Difusao/'+str(folder)+'/ssim/'+str(folder)+str(file)+'.txt','w') as arq2:
                arq2.write(str(d2))
            del(arq2)  
            ...
            d = psnr4(imgROI.astype(np.uint8), difusao_ruido.astype(np.uint8))
            d1 = mse4(imgROI.astype(np.uint8), difusao_ruido.astype(np.uint8))
            d2 = ssim4(imgROI.astype(np.uint8), difusao_ruido.astype(np.uint8),multichannel=True)
            
            with open('E:/Dissertacao/Resultados_Metricas/Filtro/Difusao_ruido/'+str(folder)+'/psnr/'+str(file)+'.txt','w') as arq2:
                arq2.write(str(d))
            del(arq2)
            with open('E:/Dissertacao/Resultados_Metricas/Filtro/Difusao_ruido/'+str(folder)+'/mse/'+str(file)+'.txt','w') as arq2:
                arq2.write(str(d1))
            del(arq2)
            with open('E:/Dissertacao/Resultados_Metricas/Filtro/Difusao_ruido/'+str(folder)+'/ssim/'+str(folder)+str(file)+'.txt','w') as arq2:
                arq2.write(str(d2))
            del(arq2)  
